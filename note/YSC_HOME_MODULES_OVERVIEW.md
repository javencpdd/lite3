...
> 范围：通过 `mcp-ssh-apply-patch` 对 `user-f20` 的 `/home/ysc` 执行只读盘点。记录时间：2026-09-06。
>
> 跳过规则：不递归展开第三方库、SDK、`lib`、`build`、`third-party`、`third_libs` 目录；仅记录其名称及用途推断。未修改远端文件、服务、网络或机器人控制状态。

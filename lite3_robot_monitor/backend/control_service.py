...
class ControlService:
    ...
    def send_velocity(self, linear_x: float, linear_y: float,
        ...
        # 取反会产出 -0.0（例如 0.0 * -1.0）。IEEE 754 里 -0.0 == 0.0，机器人行为一致，
        # 但打成字节是 0000000000000080 而非全 0，审计与排查时容易看成"发了奇怪的值"。
        # 加 0.0 即归一化（-0.0 + 0.0 == +0.0），对任何非零值无影响。
        rx, ry, raz = linear_x * sx + 0.0, linear_y * sy + 0.0, angular_z * syaw + 0.0

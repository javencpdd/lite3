...
| 目录/文件 | 职责判断 | 是否展开 |
| --- | --- | --- |
| `jy_exe/` | 厂商运动控制部署：二进制、策略、配置、服务和维护脚本 | 深度分析，见[运动控制部署分析](../运动控制与导航/运动控制部署分析.md) |
| `rtsp_stream/` | 当前活动的视频发布部署；MediaMTX + GStreamer 脚本 | 深度分析，见[实时流媒体服务分析](../视频与流媒体/实时流媒体服务分析.md) |
| `track/` | 闭源目标检测/跟随程序，使用 RKNN/RTSP/私有 UDP | 仅总体登记；当前 active |
| `host/` | 单机热点、DHCP/hostapd 与 NAT 的启动方案 | 总体分析 |
| `master/` | 多机群控热点模式的启动方案 | 总体分析 |
| `module/` | Wi-Fi 内核模块、启动分区设备树及部署脚本 | 深度分析，见[底层模块与部署分析](../运动控制与导航/底层模块与部署分析.md) |
| `slave/` | 从机侧保存 Wi-Fi 清理与连接脚本 | 深度分析，见[从机无线管理脚本](../网络与连接/从机无线管理脚本.md) |
| `qirui/` | 库集合压缩包及解压目录，无独立脚本/入口 | 深度分析，见[依赖库分析](../依赖库与SDK/依赖库分析.md) |
...
| `/home/ysc/qirui/lib` | 依赖库合集 | 第三方库 | 见[依赖库分析](../依赖库与SDK/依赖库分析.md) |
